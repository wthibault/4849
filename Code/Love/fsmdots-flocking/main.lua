...
function NPC:Avoid(them)
   ...
   if them then
      ...
      local near = bsp.withinDisk ( them, self.pos, minDist )
      if near then
...
-- flock steering - match velocities with local npcs
function NPC:MatchVel()
   local forceConstant = 5
   local minDist = 25
   local closest;
   local d;
   if Game.theGame.npcTree then

      local near = bsp.withinDisk ( Game.theGame.npcTree, self.pos, minDist )
      if near then
	 ...
	 for k,it in pairs(near) do
	    ...
	    local dist = d:len()
	    if dist > 0 and dist < minDist then
	       if it and it.id and Game.theGame.npcs[it.id] then
		  sum = sum + Game.theGame.npcs[it.id].vel
	       end
	    end
	 end
	 local diff = sum - self.vel
	 local steeringForce = diff * forceConstant / minDist*minDist
	 return steeringForce
      end
