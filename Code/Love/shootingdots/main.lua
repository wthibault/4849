...
function Moving:enteredState()
   ...
   love.audio.play(self.sound)
   self.intersection = vector(0,0)
end

function findIntersection ( N, c, P, D )
   local u = 10000
   local denom = N * D
   if denom ~= 0 then
      u = (-c - N * P ) / denom
...
function Moving:update(dt)
   ...
   --   self.radius = 8 + 4*(math.sin(9*math.pi * elapsedTime)) -- 9Hz flutter
   self.radius =  4+4 *math.abs(math.sin(9*math.pi * elapsedTime)) -- 9Hz flutter
   local steeringForce = self:Avoid ( Game.theGame.bullets )

   ------------------------- compute intersection with walls, pick nearest
   wallNormals = { vector(1,0), vector(0,1), vector(-1,0), vector(0,-1) }
   wallCs = { 0,0,799,599 }
   smallestU = 1000000
   wallHit = 0
   for i=1,4 do
      u = findIntersection ( wallNormals[i], wallCs[i], self.pos, self.vel )
      if u > 0 and smallestU > u then
	 smallestU = u
	 wallHit = i
      end
   end
   self.intersection = self.pos + smallestU * self.vel;
   print ( self.intersection )
     
  ------ createe the proxy sna seek it
   local proxyForce;
   if  wallHit > 0 then
      self.proxy = self.intersection + 50 * wallNormals[wallHit]
      proxyForce =  self.proxy - self.pos
      proxyForce = proxyForce * 10
   end

   if steeringForce then
      steeringForce = steeringForce + proxyForce
   else
      steeringForce = proxyForce
   end
...
function Moving:draw()
   NPC.draw ( self )
   local ahead = self.pos + self.vel * 1.5
   love.graphics.line ( self.pos.x, self.pos.y, ahead.x, ahead.y )
   love.graphics.setPointSize ( 10 )
   love.graphics.setColor ( 255,255,255,255 )
   love.graphics.point ( self.intersection.x, self.intersection.y )
   if self.proxy then
      love.graphics.point (self.proxy.x, self.proxy.y )
   end
end
